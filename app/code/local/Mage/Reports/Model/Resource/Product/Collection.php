...
class Mage_Reports_Model_Resource_Product_Collection extends Mage_Catalog_Model_Resource_Product_Collection
{
    ...
    public function getSelectCountSql()
    {
        if ($this->_selectCountSqlType == self::SELECT_COUNT_SQL_TYPE_CART) {
            ...
            $countSelect->reset()->from(array(
                ...
                ), 'quote_table.entity_id = quote_item_table.quote_id AND quote_table.is_active = 1', array());
            return $countSelect;
        }
    ...
    public function addCartsCount()
    {
        ...
        $countSelect->from(array(
            ...
            ), 'quotes.entity_id = quote_items.quote_id AND quotes.is_active = 1', array())->where("quote_items.product_id = e.entity_id");

        $this->getSelect()->columns(array(
            "carts" => "({$countSelect})"
        ))->group("e.{$this->getProductEntityId()}")->having('carts > ?', 0);
    ...
    public function addOrdersCount($from = '', $to = '')
    {
        ...
        ), "order_items.product_id = {$productFieldName}", array())->columns(array(
                'orders' => 'COUNT(order_items2.item_id)'
            ))->group($productFieldName);

        $dateFilter = array(
            'order_items2.item_id = order_items.item_id'
        );
        if ($from != '' && $to != '') {
    ...
    public function addOrderedQty($from = '', $to = '')
    {
        ...
        $subSelect       = $this->getSelect()->reset()->from(array(
            ...
            ), implode(' AND ', $orderJoinCondition), array())->joinLeft(array(
                'cpr' => $this->getTable('catalog/product_relation')
            ), 'cpr.child_id = order_items.product_id', array(
                'parent_id'
            ))->joinLeft(array(
                'cat_index' => $this->getTable('catalog/category_product_index')
            ), 'cat_index.product_id = order_items.product_id', array(
                'store_id',
                'visibility',
                'category_id'
            ))->where('parent_item_id IS NULL')->where('cat_index.store_id = ?', Mage::app()->getStore()->getId())->where('category_id = ?', Mage::app()->getStore()->getRootCategoryId());
        $subSelectString = '(' . $subSelect->__toString() . ')';
        $this->getSelect()->reset()->from(array(
            'T1' => new Zend_Db_Expr($subSelectString)
        ), array(
            ...
            ), implode(' AND ', $productJoinCondition), array(
    ...
    public function addViewsCount($from = '', $to = '')
    {
        ...
        $this->getSelect()->reset()->from(array(
            ...
            'views' => 'COUNT(report_table_views.event_id)'
                ...
                'e' => $this->getProductEntityTableName()
            ), $this->getConnection()->quoteInto("e.entity_id = report_table_views.object_id AND e.entity_type_id = ?", $this->getProductEntityTypeId()))->where('report_table_views.event_type_id = ?', $productViewEvent)->group('e.entity_id')->order('views ' . self::SORT_ORDER_DESC)->having('COUNT(report_table_views.event_id) > ?', 0);
